...
public class analyzeGrammar {
    ...
    public String analyzeGrammarLLO(String s) throws Exception {
        CreateLL1Table();//创建LL(1)分析表
        int tokenLength = createToken.token.size();
        grammarDisplay.append("分析栈            输入流当前匹配元素\n");
    ...
    public int analyze(int i) {
        ...
        grammarDisplay.append(showStack()+ "            "+iToken+"\n");
        ...
        if (nonterminator.contains(iStack2)) { // 栈顶元素为非终极符，则要根据LL(1)分析表进行规约
            ...
            if ((pnum = analysis[nonterminator.indexOf(iStack2)][terminator.indexOf(iToken)]) == 0) { // 出错！因没有可用的规则进行规约
                grammarDisplay.append("行:" + iLine + "			" +  "语法错误，没有语法规则匹配\n");
                stop = true;
                return 0;
            } else {
            ...
            if (iStack2.equals(iToken)) { // 匹配成功
                ...
                treeDepth = stack2.pop();
                nextLoop = true;
                String mat = "Match ( "+iStack2 +" , "+iToken+" )\n";
                grammarDisplay.append(mat);
                if (iToken.equals(".")) {
                    nextLoop = false;
                    stop = true;
                    isTokenEmpty = true;
                ...
                grammarDisplay.append("行:" + iLine + "			" +  "语法错误，意外字符\n");
                stop = true;
                return 0;
            }
        }
